    ...
    <>
        <div className={`container-md cart-container bg-white shadow ${active ? 'cart-active' : ''}`}>
            ...
            <div className="row border-top mb-3">
              {
                orders.length === 0 && <div className='col-md-12 py-3'>
                  <p>Your Cart is empty.</p>
                  <Link to='/Shop' onClick={()=>setActive(!active)} className="btn btn-secondary">SHOP NOW</Link>
                </div>
              }
                  ...
                  <div className="cart-item-info d-flex justify-content-between text-capitalize">
                    <div>
                      ...
                      <div className="cart-qty-container">
                        <span className='qty-btn' onClick={()=>handleMinus(index)}>-</span>
                        <input type="number" name="qty" id="cartQty" className='cart-input w-25 text-center mx-1' min="1" value={order.quantity} onChange={(event) => handleQuantityChange(index, event)} />
                        <span className='qty-btn' onClick={()=>handlePlus(index)}>+</span>
                      </div>
                    </div>
                    <div>
                      <button className="btn" onClick={()=>handleDel(index)}>{<FaTrash />}</button>
                    </div>
                  </div>
            ...
            {orders.length > 0 ? 
              <>
                ...
                <button className='btn btn-secondary text-uppercase' onClick={()=>handleCheckout()}>Proceed to Checkout</button>
              </>
            : ""}
